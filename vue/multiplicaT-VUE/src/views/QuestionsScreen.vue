<template>
    <v-container fluid style="border: 1px solid;">
        <v-row justify="center" style="border: 1px solid; height: 100vh;">
            <v-col cols="12">
                <v-container v-if="!showQuestions">
                    <v-btn color="primary" @click="startGame">Iniciar Partida</v-btn>
                    <h2>Contador: {{ countdown }}</h2>
                </v-container>
                <v-container v-if="showQuestions" class="box-game">
                    <h1>Pregunta {{ currentQuestionIndex + 1 }} de {{ this.questions.length
                    }}:</h1>
                    <v-row>
                        <v-col>
                            <v-container>
